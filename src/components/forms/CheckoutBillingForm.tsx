...
// import Countries from "@/src/lib/json/country.json";
// import makeApiCallService from "@/src/lib/service/apiService";
// import { billingAtom } from "@/src/lib/storage/jotai";
...
export function CheckoutBillingForm() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setBillingInfo(response?.response?.data ? response.response.data as unknown as BillingInfo : null);
    } catch (error) {
      console.error("Error saving billing info:", error);
    } finally {
      setIsLoading(false);
