...
interface ProductDetailExtraInfoSectionProps {
  product_id: string;
}

const ProductDetailExtraInfoSection: React.FC<ProductDetailExtraInfoSectionProps> = ({ product_id }) => {
  const [desc, setDescription] = useState<any>(null);
  ...
  return (
    <div>
      ...
      {desc.imageUrl && <Image src={desc.imageUrl} alt="Product Image" width={500} height={500} />}
    </div>
  );
};
